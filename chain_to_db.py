...
class Nijigasaki(object):
    ...
    def _make_triplet(self, morphemes):
        ...
        # beginを追加
        triplet = (Nijigasaki.BEGIN, morphemes[0], morphemes[1])
        triplet_freqs[triplet] = 1

        # endを追加
        triplet = (morphemes[-2], morphemes[-1], Nijigasaki.END)
        triplet_freqs[triplet] = 1
    ...
    def save(self, triplet_freqs, init=False):
        ...
        """
        # DBオープン
        con = sqlite3.connect(Nijigasaki.DB_PATH)

        # 初期化から始める場合
        if init:
            # DBの初期化
            with open(Nijigasaki.DB_SCHEMA_PATH, "r") as f:
                schema = f.read()
        ...
        # データ挿入
        p_statement = "insert into nijigasaki (prefix1, prefix2, suffix, freq) values (?, ?, ?, ?)"
...
class TestFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.chain = Nijigasaki(self.text)
